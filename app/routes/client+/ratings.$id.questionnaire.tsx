...
export default function RatingQuestionnaire() {
  ...
  const FetcherData = Fetcher.data as { storedUrl: string; message: string; error: string; delete: string };
  ...
  const onConfirmDelete = (name: string | null) => {
    ...
    const file = questions[activeQuestions[0].Header as keyof typeof questions].find((response) => response.id === name)
      ?.Response?.file;
  ...
  useEffect(() => {
    if (FetcherData?.storedUrl) {
      ...
      setQuestions((prev) => ({ ...prev, [activeQuestions[0].Header]: newResponses }));
    ...
    if (FetcherData?.delete) {
      ...
      const newResponses = questions[activeQuestions[0].Header as keyof typeof questions].map((response) => {
        if (response.id === questionId && response?.Response) response.Response.file = null;
        return response;
      });
      setQuestions((prev) => ({ ...prev, [activeQuestions[0].Header]: newResponses }));
    ...
    if (FetcherData?.message) toast.success(FetcherData?.message, { toastId: 'update_rating' });
    if (FetcherData?.error) toast.error(FetcherData?.error, { toastId: 'update_rating' });
  }, [FetcherData]);
    ...
    <div className="flex flex-col flex-1 h-full gap-2">
      ...
      <dialog className="modal" ref={ConfirmRef}>
        <div className="px-6 py-6 rounded-lg bg-base-100">
          ...
          <p className="py-2 text-sm">Are you sure you want to delete this file?</p>
          <div className="modal-action">
            <div className="flex gap-2">
              <button onClick={() => ConfirmRef.current?.close()} disabled={isSubmitting} className="btn btn-sm">
                Cancel
              </button>
